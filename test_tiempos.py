...
# Expresión regular para encontrar la prueba, serie, género y categoría
prueba_serie_genero_categoria = r'(\d+)\s+([A-Z\s]+)\s*:\s*([A-Z\s]+)?\s*-?([A-Z]+)?'

# Buscar coincidencias en el texto
coincidencias = re.findall(prueba_serie_genero_categoria, texto)

# Imprimir las coincidencias encontradas
for coincidencia in coincidencias:
    print("Prueba:", coincidencia[0])
    print("Serie:", coincidencia[1])
    print("Género:", coincidencia[2] if coincidencia[2] else "No especificado")
    print("Categoría:", coincidencia[3] if coincidencia[3] else "No especificada")
    print()
